...
def custom_loss(y_true, y_pred, mode):
    if mode==0:
        w_L = 1.0
    else:
        w_L = 0.2

    loss = -w_L*K.sum((y_true*K.log(y_pred)+(1-y_true)*K.log(1-y_pred)))
...
for ro in range(1):# several rounds to test the stablity of the model OR accumulate the model's performance
  ...
  #Train the specific cell line
    ...
    for cell_line_test in cell_lines_tests:# monitor the process of training on all the cell lines to make a general model and test on all the cell lines
        ...
        np.save(('Basic_general_y_predict_Batch'+str(batch_size)+'_Kernel'+str(kernel_size)+cell_line+'_test'+cell_line_test+str(ro)), y_score)
